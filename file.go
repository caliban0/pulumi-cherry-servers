...
func (*File) Create(
	...
	if !req.Inputs.Force {
		...
		if !os.IsNotExist(err) {
			return resp, errors.New("file already exists; pass force=true to override")
		}
	}
...
func (*File) Update(
	...
	if !req.DryRun && req.State.Content != req.Inputs.Content {
		...
		if n != len(req.Inputs.Content) {
			return infer.UpdateResponse[FileState]{}, fmt.Errorf(
				"only wrote %d/%d bytes",
				n,
				len(req.Inputs.Content),
			)
		}
	}
